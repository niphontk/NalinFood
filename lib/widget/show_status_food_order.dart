...
class _ShowStatusFoodOrderState extends State<ShowStatusFoodOrder> {
  ...
  Widget buildContent() => ListView.builder(
        ...
        itemBuilder: (context, index) => Column(
          children: [
            ...
            buildTotal(index),
            MyStyle().mySizebox(),
            buildIndicator(statusInts[index]),
            MyStyle().mySizebox(),
          ],
        ),
      );

  Widget buildIndicator(int index) => Column(
        children: [
          StepsIndicator(
  ...
  Row buildTransport(int index) {
    return Row(
      children: [
        MyStyle().showTitleH3Purple('ค่าขนส่ง ${orderModels[index].transport}'),
      ],
    );
  }
  ...
  Center buildNonOrder() => Center(child: Text('ไม่มีประวัติสั่งอาหาร'));
  ...
  Future<Null> readOrderFromIdUser() async {
    if (idUser != null) {
      String url =
          '${MyConstant().domain}/nalinfood/getOrderWhereIdUser.php?isAdd=true&idUser=$idUser';
